...
async function getBookingDetails(bookingId: string) {
  ...
  console.log('BOOKING USER:', booking.user_id)
  console.log('HOTEL OWNER:', booking.rooms?.hotels?.owner_id)
  
  // Authorization logic
  const isGuest = booking.user_id === user.id
  const isOwner = booking.rooms?.hotels?.owner_id === user.id
...
function BookingNotFound() {
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-black">
      <header className="bg-white dark:bg-zinc-900 shadow-sm border-b border-gray-200 dark:border-zinc-800 no-print">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-16">
            <Link 
              href="/account/bookings" 
              className="inline-flex items-center text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white font-medium transition-colors"
            >
...
export default async function BookingDetailsPage({ 
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-black print-white-bg">
      ...
      <main className="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        {/* Actions */}
        <div className="mt-6 border-t border-gray-200 dark:border-zinc-700 pt-6 no-print">
